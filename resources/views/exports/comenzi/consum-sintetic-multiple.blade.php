@php
    $formatQuantity = static function ($value) {
        $formatted = number_format((float) $value, 4, '.', '');
        return rtrim(rtrim($formatted, '0'), '.') ?: '0';
    };
    $rows = collect($report['summary_rows'] ?? []);
@endphp
<table border="0">
    <tr>
        <td colspan="7" style="font-size: 18px; font-weight: bold;">{{ $report['title'] ?? 'FISA SINTETICA COMENZI-CONSUMURI' }}</td>
    </tr>
    <tr>
        <td colspan="7" style="font-weight: bold;">PERIOADA : {{ $report['period_label'] ?? '-' }}</td>
    </tr>
    <tr><td colspan="7"></td></tr>
    <tr>
        <td style="font-weight: bold;">Nr. de comenzi</td>
        <td>{{ $report['order_count'] ?? 0 }}</td>
        <td style="font-weight: bold;">Nr. clienti</td>
        <td>{{ $report['client_count'] ?? 0 }}</td>
        <td style="font-weight: bold;">Clienti noi</td>
        <td>{{ $report['new_client_count'] ?? 0 }}</td>
        <td></td>
    </tr>
    <tr>
        <td style="font-weight: bold;">Operator</td>
        <td>{{ $report['generated_by'] ?? '-' }}</td>
        <td style="font-weight: bold;">Data listarii fisei</td>
        <td>{{ optional($report['generated_at'] ?? null)->format('d.m.Y H:i') ?? '-' }}</td>
        <td></td>
        <td></td>
        <td></td>
    </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5" style="margin-top: 16px; border-collapse: collapse;">
    <thead>
        <tr style="background: #eef2f7; font-weight: bold;">
            <th>#</th>
            <th>Material</th>
            <th>UM</th>
            <th>Echipament utilizat</th>
            <th>Consum</th>
            <th style="background: #fff1f1;">Rebut</th>
            <th>Total</th>
        </tr>
    </thead>
    <tbody>
        @forelse ($rows as $row)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $row['material'] }}</td>
                <td>{{ $row['unitate_masura'] }}</td>
                <td>{{ $row['echipamente'] }}</td>
                <td>{{ $formatQuantity($row['consum']) }}</td>
                <td style="background: #fff1f1;">{{ $formatQuantity($row['rebut']) }}</td>
                <td>{{ $formatQuantity($row['total']) }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="7" style="text-align: center;">Nu exista materiale de centralizat.</td>
            </tr>
        @endforelse
    </tbody>
</table>
